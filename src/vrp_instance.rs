...
use kiddo::{distance_metric::DistanceMetric, float::kdtree::KdTree, SquaredEuclidean};
use ordered_float::OrderedFloat;

use crate::{params::Params, polar_sector};
...
impl VRPInstance {
    pub fn new(file: String, params: Params) -> VRPInstance {
        ...
        for c in customers.iter() {
            let (x, y) = (c.x, c.y);
            let mut nn = kdtree
                .nearest_n::<SquaredEuclidean>(&[x, y], params.gamma)
                .iter()
                .map(|x| customers[x.item])
                .collect::<Vec<_>>();
...
pub struct Customer {
    ...
    pub y: f64,
    pub demand: f64,
    // Polar angle of client around depot the depot, in truncated degrees. For degree computation,
...
impl Display for Customer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Customer({}, {}, {}, {}, {})",
            self.id, self.x, self.y, self.demand, self.polar_angle
        )
    }
}
