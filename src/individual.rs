...
use std::{collections::BinaryHeap, fs, path::PathBuf, sync::Arc};
...
impl Individual {
  ...
  pub fn new(vrp: Arc<VRPInstance>) -> Individual {
    ...
    // Note that n_customers excludes the depot, so add 1
  ...
  /// Compute objective as distance traveled, plus a penalty for excess capacity
  pub fn objective(&mut self) -> f64 {
    ...
    self.objective = self.total_dist + (self.excess_cap as f64 * self.vrp.params.excess_penalty);
    log::debug!("Total distance: {:.2}, excess capacity: {}, objective: {:.2}", self.total_dist, self.excess_cap, self.objective);
    self.objective
  }
  ...
  /// Splits the total route into vehicle routes using Bellman's algorithm in topological order.
  /// Used not only in feasible solution instantiation, but also in genetic crossover.
        ...
        if cost_to_c[v][c] < INF {
          ...
          for next in (c + 1)..(vrp.n_customers + 1) {
            ...
            if load >= 1.5 * vrp.vehicle_cap as f64 {
              log::trace!("Load {} too high for vehicle {}; stopping at {}", load, v, next);
              break;
            }
            ...
            // If next is c+1, use distance from depot; otherwise, use distance from previous customer
            if next == c + 1 {
            ...
            // Compute cost using current distance + distance from here back to depot + a penalty if load is too high
            let cost = dist + vrp.dist_mtx[next][0]
  ...
  /// Initializes a (potentially infeasible) solution using parallel greedy NN search.
  ...
  /// Uses the Clark-Wright savings algorithm to attempt to generate a feasible solution.
  pub fn cw_savings(&mut self) -> bool {
    ...
    for i in 1..(self.vrp.n_customers + 1) {
      for j in (i+1)..(self.vrp.n_customers + 1) {
        let dist = self.vrp.dist_mtx[0][i] + self.vrp.dist_mtx[j][0] - self.vrp.dist_mtx[i][j];
        savings[i][j] = dist;
        savings[j][i] = dist;
      }
    }
    ...
    // - Otherwise, find in existing routes the best place to insert each unassigned customer
